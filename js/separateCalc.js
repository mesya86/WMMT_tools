...
// 分身
const onlineGhostCountSeparate = [
...
/* メソッド */
// ランク
/*function getRank(story, winCount, VSPlayer, storyCountSeparate, onlineGhostCountSeparate, VSPlayerCountSeparate, rank) {
    // 値以下の最大インデックスを返す
    ...
    const idxStory = match(story, storyCountSeparate) + 1;
    const idxOnlineGhost = match(winCount, onlineGhostCountSeparate) + 1;
    const idxVSPlayer = match(VSPlayer, VSPlayerCountSeparate) + 1;
...
function calcStoryRank(story) {
    // 配列の最大値を取得
    const maxValue = storyCountSeparate[storyCountSeparate.length - 1];
    ...
    if (story < 10) {
        ...
        // 最大値を探す
        let index = -1;
        for (let i = 0; i < storyCountSeparate.length; i++) {
            if (storyCountSeparate[i] <= story) {
                index = i;
            } else {
                break;
            }
        ...
        const next = (index + 1 < storyCountSeparate.length) ? storyCountSeparate[index + 1] : storyCountSeparate[storyCountSeparate.length - 1];
...
function calconlineGhostRank(ghostDefeated, totalDefeated) {
    // 配列の最大値を取得
    const maxValue = onlineGhostCountSeparate[onlineGhostCountSeparate.length - 1];
    ...
    if (winCount < 30) {
        ...
        // 最大値を探す
        let index = -1;
        for (let i = 0; i < onlineGhostCountSeparate.length; i++) {
            if (onlineGhostCountSeparate[i] <= winCount) {
                index = i;
            } else {
                break;
            }
        ...
        // 次のランク基準（index + 1 の値）を取得
        const next = (index + 1 < onlineGhostCountSeparate.length) ? onlineGhostCountSeparate[index + 1] : onlineGhostCountSeparate[onlineGhostCountSeparate.length - 1];
...
function calcVSPlayer(VSPlayer) {
    // 配列の最大値を取得
    const maxValue = VSPlayerCountSeparate[VSPlayerCountSeparate.length - 1];
    ...
    if (VSPlayer < 20) {
        ...
        // 最大値を探す
        let index = -1;
        for (let i = 0; i < VSPlayerCountSeparate.length; i++) {
            if (VSPlayerCountSeparate[i] <= VSPlayer) {
                index = i;
            } else {
                break;
            }
        ...
        // 次のランク基準（index + 1 の値）を取得
        const next = (index + 1 < VSPlayerCountSeparate.length) ? VSPlayerCountSeparate[index + 1] : VSPlayerCountSeparate[VSPlayerCountSeparate.length - 1];
...
document.addEventListener('DOMContentLoaded', () => {
    const toggle = document.getElementById('toggleExtraBtn');
    ...
    toggle.addEventListener('change', () => {
        const display = toggle.checked ? 'inline-block' : 'none';
        storyBtn.style.display = display;
        ghostBtn.style.display = display;
        vsBtn.style.display = display;
    });
